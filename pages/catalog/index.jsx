  ...
  return (
    <div>
      ...
      <div className="cat-filter">
        <div className="container">
          <div className="cat-filter__content">{<CatalogFilters filtersList={filtersList} />}</div>
        </div>
      </div>
              ...
              return (
                <div key={index} className="catalog__item">
                  <div className="catalog__item-img">
                    <Image width="384px" height="256px" src={element.product?.shop_product_images?.[0]?.image ? 'https://trade-group.su/' + element.product?.shop_product_images?.[0]?.image : '/no-image.png'} alt="" />
                  </div>
                  ...
                  <div className="catalog__item-info">
                    {
                      element.product.shop_product_properties
                        .map((element, index) => {
                          return (
                  ...
                  <div className="catalog__item-btns">
                    <a onClick={() => router.push(`/catalog/${element.product.slug}`)} className="catalog__item-more-btn">
                      Подробнее
                    </a>
                    <button onClick={() => orderProductClickHandler(element.product)} className="catalog__item-btn" data-modal>
                      Оставить заявку
                    </button>
                  </div>
                </div>
...
export async function getServerSideProps(context) {
  ...
  if (!query['categories']) {
    // если пользователь задал не категорию
  ...
  const productsFilters = query['filters']
    ? query['filters']
        .split(';')
        .map((filter, index, arr) => {
          ...
          if (filterParams[1].split(',').length >= 2) {
            // если в значении фильтра указано 2 или больше параметра, например "тип двигателя:Дизель,Электрический"
  ...
  const productsURI = quickLink ? encodeURI(`https://trade-group.su/apicatalog?${quickLink}`) : encodeURI(`https://trade-group.su/apicatalog?categories=${category}&${productsFilters}page=${page}`);
