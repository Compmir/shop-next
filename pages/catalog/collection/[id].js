            ...
            <div className="catalog">
                <div className="container">
                    <div className="catalog__wrapper">
                                ...
                                return (
                                    <div key={index} className="catalog__item">
                                        ...
                                        <div style={{visibility: "hidden"}} className="catalog__item-favorite-btn">
                                            <svg xmlns="http://www.w3.org/2000/svg" height="28px" viewBox="0 0 24 24"
                                                 width="28px" fill="gray">
                                        ...
                                        <div className="catalog__item-info">
                                            {
                                                element.properties.map((element, index) => {
                                                    ...
                                                    return (
                                                        <div key={index}
                                                             className="catalog__item-info-block">
                                                            <div
                                                                className="catalog__item-info-right">{property}</div>
                                                            <div
                                                                className="catalog__item-info-left">{value}</div>
                                                        </div>
                                                    )
                                        ...
                                        <div className="catalog__item-btns">
                                            <a onClick={() => router.push(`/catalog/${element.slug}`)}
                                               className="catalog__item-more-btn">Подробнее</a>
                                            <button onClick={() => orderProductClickHandler(element)}
                                                    className="catalog__item-btn" data-modal>Оставить заявку
                                            </button>
                                        </div>
                                    </div>
                                )
						...
						{
                            !goods.length &&
                            <p style={{textAlign: "center", color: "#fff", fontSize: "24px"}}>Товары не найдены!</p>
                        }
                    </div>

                    {/*<CatalogPaginate pagesCount={pageCount}/>*/}

                    <OrderCallModal closeModal={() => setModalShowStatus(false)} isOpen={isModalShow}
