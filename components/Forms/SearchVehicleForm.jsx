...
export const SearchVehicleForm = ({ delivery, deliveryFeedbackItems }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  if (delivery) {
    return (
      <form onSubmit={handleSubmit(submitHandler)}>
        ...
        <div className="delivery__form-item" ref={(e) => (deliveryFeedbackItems.current[1] = e)}>
          <input {...register('phone', { required: true })} className="delivery__form-input" type="text" name="phone" placeholder="Ваш номер телефона" />
        </div>
        <div className="delivery__form-item" ref={(e) => (deliveryFeedbackItems.current[2] = e)}>
          <input {...register('name', { required: true })} className="delivery__form-input" type="text" name="name" placeholder="Ваше имя"></input>
        </div>
        <div style={{ marginTop: '10px' }} className="delivery__form-item" ref={(e) => (deliveryFeedbackItems.current[3] = e)}>
          <button className="delivery__form-btn intro__btn-red-hover" type="submit">
            Отправить
          </button>
        </div>
        <div className="delivery__form-item" ref={(e) => (deliveryFeedbackItems.current[4] = e)}>
          <div className="delivery__form-checkbox">
            ...
            <label htmlFor="formAgreement" className="delivery__checkbox-label">
              <span>Я даю свое согласие на обработку персональных данных и соглашаюсь с условиями и политикой конфиденциальности</span>
            </label>
          </div>
        </div>
  ...
  return (
    <form onSubmit={handleSubmit(submitHandler)}>
      ...
      <div style={{ marginTop: '10px' }} className="map__form-item">
        <button className="map__form-btn intro__btn-red-hover" type="submit">
          Отправить
        </button>
      </div>
      <div className="delivery__form-item">
